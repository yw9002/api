...
public class SolrClientKit {
	...
	public static SolrClient getHttpSolrCientInstance(){
		if(httpSolrClinet == null){
			return new HttpSolrClient(Cache.getSetting(Const.SOLR_URL).getValue());
		}
	...
	public static SolrClient getConcurrentUpdateSolrClientInstance(){
		if(concurrentUpdateSolrClient == null){
			return new ConcurrentUpdateSolrClient(Cache.getSetting(Const.SOLR_URL).getValue(), Integer.parseInt(Cache.getSetting(Const.SOLR_QUEUESIZE).getValue()), Integer.parseInt(Cache.getSetting(Const.SOLR_THREADCOUNT).getValue()));
		}
